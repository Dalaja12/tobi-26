...
// Emociones automáticas — cambio seguro cada 10 segundos
...
function startAutoEmotions(intervalMs = 10000) {
    ...
    autoEmotionsInterval = setInterval(() => {
        ...
        // Elegir una emoción aleatoria distinta a la actual
        let newEmotion = emotions[Math.floor(Math.random() * emotions.length)];
        if (newEmotion === currentMood) {
            // si salió igual, tomar la siguiente del array (garantiza cambio)
...
function takeSelfie() {
    ...
    setTimeout(() => {
        ...
        // ⚠️ QUITAMOS EL FLASH ANTES DE CAPTURAR
        document.body.classList.remove('camera-flash');

        // Pequeñísimo delay para asegurar que no esté en el DOM
        setTimeout(() => {
    ...
    }, 200); // duración visible del flash
}
...
async function searchWeb(query) {
    ...
    // 🔁 SI NO HAY TEXTO → BUSCAR TÍTULO CORRECTO
...
function getHumanIntro() {
    const phrases = [
        ...
        "Esto es lo que encontré sobre eso 👇",
...
userInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        ...
        hideAutocomplete();     // 👈 CIERRA AUTOCOMPLETADO
        sendQuestion();         // envía tu pregunta
    }
});
...
function playAnswer() {
    ...
    // ▶️ MÓVIL o NUEVA REPRODUCCIÓN → HABLAR DESDE CERO
